<div align="center">
  <h1>
    Hi, I'm Muhammad
  </h1>
</div>

<div align="center">
  <h4>Full-stack developer focused on building scalable web applications with modern technologies.</h4>
</div>

---

### Core Stack & Technologies

<table align="center">
  <tr>
    <td align="center" width="18%">
      <img src="https://skillicons.dev/icons?i=ts,js" /><br />
      <b>Languages</b>
    </td>
    <td align="center" width="18%">
      <img src="https://skillicons.dev/icons?i=nextjs,react,tailwind" /><br />
      <b>Frontend</b>
    </td>
    <td align="center" width="18%">
      <img src="https://skillicons.dev/icons?i=nodejs,express,bun" /><br />
      <b>Backend</b>
    </td>
    <td align="center" width="18%">
      <img src="https://skillicons.dev/icons?i=postgres,mongodb,prisma,supabase" /><br />
      <b>Databases</b>
    </td>
    <td align="center" width="18%">
      <img src="https://skillicons.dev/icons?i=github,docker,linux,neovim,vercel,cloudflare" /><br />
      <b>Tools</b>
    </td>
  </tr>
</table>

---

###  Current Projects:

*    **[ZeroZone](https://github.com/DevMuhammed3/ZeroZone)** - A full-stack, live communication web application built inside a **Turborepo Monorepo** (`apps/web` & `packages/ui`).
    *   *Architecture:* Next.js, TypeScript, Tailwind CSS, Socket.io.
    *   *Status:* Deployed on Vercel, currently patching and optimizing performance.
*    **[PortfolioBuilder](https://github.com/DevMuhammed3/PortfolioBuilder)** - An application designed to build and generate personalized professional portfolios.
    *   *Status:* Codebase established, currently in active development.
*    **[Media Tracker](https://github.com/DevMuhammed3/WatchStash)** *(In Development)* - A high-performance media tracking app for movies, series, and anime.
    *   *Architecture:* Bun, Express, TypeScript.

---

### About:

- Full-stack developer from **Egypt**.
- Working primarily with TypeScript.
- Interested in Backend Engineering.
- Currently learning System Design and Distributed Systems.

---

### Currently Learning:

- Docker
- CI/CD
- Testing
- System Design
- Kubernetes

---

### GitHub Activity

<div align="center">
  <br />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=DevMuhammed3&theme=tokyonight" alt="Muhammed's Streak" />
</div>

---


###  Connect:

<p align="center">
  <a href="https://github.com/DevMuhammed3">
    <img src="https://img.shields.io/badge/GitHub-0B1220?style=for-the-badge&logo=github&logoColor=white" />
  </a>
  <a href="mailto:muhdid82@gmail.com">
    <img src="https://img.shields.io/badge/Email-muhdid82@gmail.com-0B1220?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</p>
